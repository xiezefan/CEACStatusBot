...
def query_status(location, application_num, passport_number, surname, captchaHandle: CaptchaHandle = OnnxCaptchaHandle("captcha.onnx")):
    ...
    def truncate(text: str) -> str:
        # Limit log size for easier debugging and to avoid overly large outputs.
        return text if len(text) <= max_log_chars else text[:max_log_chars]

    while not isSuccess and failCount < 2:
        failCount += 1
        headers = {
